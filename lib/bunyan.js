 ...
 *
 * @param callback {function} Called after all the current streams have been
 *     closed, so that it is safe to perform further processing on the rotated
 *     log files. For example the files could be compressed and transferred to
 *     to another location.
 */
Logger.prototype.reopenFileStreams = function (callback) {
    var self = this;
    ...
    self.streams.forEach(function (s) {
        if (s.type === 'file') {
            if (s.stream) {
                streamCount++;
                s.stream.on('close', onStreamClosed);
